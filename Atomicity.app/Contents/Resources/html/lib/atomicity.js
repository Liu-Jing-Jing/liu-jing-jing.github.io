...
var Person = requireFunc('lib/tab');
console.log(Person);
  var views = document.getElementsByClassName('tabscon')[0];
  var frame = getClient();
  console.log(frame);
  views.style.height = frame.clientHeight-50 + 'px';
  function getClient() {
      return {
          /**
           * 获得页面的可视区域
  ...
  Chrome = {
    ...
    openPanel: function() {
      ...
      if (panel.runModal !== OSX.NSFileHandlingPanelOKButton) {return null;}
  ...
  Chrome.bindKey('drawer', 'Ctrl-Command-N', function(env) {
    //Project.tiggle();  显示或隐藏项目列表的部分
  });
  // ace modify
